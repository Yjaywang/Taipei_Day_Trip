...
let view = {
  ...
  render: function (response) {
    ...
    const profilePicContainerEls = document.querySelectorAll(
    ...
    response.images.forEach((image) => {
      ...
      imgProfile.addEventListener("load", function (e) {
        ...
        if (imgCount === response.images.length) {
          ...
          profilePicContainerEls[0].insertBefore(
            divSlideContainer,
            profilePicContainerEls.lastElementChild
          );
          profilePicContainerEls[0].insertBefore(
            divSlideBtnContainer,
            profilePicContainerEls.lastElementChild
          );
          profilePicContainerEls[0].insertBefore(
            divCircleContainer,
    ...
    circleEls.forEach((circleEl) => {
      circleEl.setAttribute(
        "onclick",
        `controller.currentSlide(${String(i + 1)})`
      );
    });
