...
let model = {
  ...
  deleteBooking: async function () {
    document.addEventListener("click", async function (e) {
      const attractionID = e.target.parentElement
        .querySelector(".booking-attraction-link")
        .href.split("/")
        .slice(-1)[0];
      const date =
        e.target.parentElement.querySelector(".booking-date").childNodes[1]
          .textContent;
      const tempTime =
        e.target.parentElement.querySelector(".booking-time").childNodes[1]
          .textContent;
      let time = "";
  ...
  submitBooking: function submitBooking(event) {
    ...
    TPDirect.card.getPrime(async function (result) {
      ...
      const bookingSubContainerEls = document.querySelectorAll(
        ".booking-info-sub-container"
      );
      ...
      const userEmail =
        userContainerEl.querySelector(".user-email input").value;
      const userPhone =
        userContainerEl.querySelector(".user-phone input").value;
      let trip = [];
      const contact = {
        name: userName,
        email: userEmail,
      ...
      bookingSubContainerEls.forEach((bookingSubContainerEl) => {
        const attractionID = parseInt(
          bookingSubContainerEl.querySelector("a").href.split("/").slice(-1)
        );
        const attractionName =
          bookingSubContainerEl.querySelector(".attraction").textContent;
        const address = bookingSubContainerEl.querySelector(
          ".booking-address-content"
        ).textContent;
        const image = bookingSubContainerEl.querySelector(".booking-img").src;
        const date = bookingSubContainerEl.querySelector(
          ".booking-date-content"
        ).textContent;
        const temp_time = bookingSubContainerEl.querySelector(
